...
namespace MyApp.REPLServer.WebSocketServerNamespace
{
    ...
    //   - `websocket-sharp` seems to be a popular one (http://sta.github.io/websocket-sharp/).
    //   - I chose to handroll this one on top of System.Net.Sockets.TcpListener because I didn't
    //     want to introduce any dependencies into the project for this development tool. The
    //     implementation is incomplete and not production quality but it has worked great as a part
    //     of this development tool.
    //
    // In fact, you don’t even have to use WebSockets -- you can wire the REPL server and client
    // together using your preferred communication mechanism.
    internal class WebSocketServer : IDisposable
    {
        ...
        {
            var message = new JsonArray(cmd, JsonSerializer.Serialize(args)).ToString();
        ...
        public Task SendMessage(string message)
        {
            ...
            foreach (var client in clients)
            {
                try
                {
                    LogOnFailure(client.SendMessage(message, _cts.Token));
                }
                catch (Exception ex)
                {
                    LogInfo($"WebSocketServer.SendMessage error: {ex}");
                }
            }
        ...
        private async Task StartServerLoop()
        {
            ...
            try
            {
                while (!_cts.IsCancellationRequested)
                {
                    ...
                    LogOnFailure(HandleClient(client));
                }
            }
            ...
            {
                {
                    ...
                    {
                        LogInfo($"WebSocketServer: Expected Origin \"localhost\" but found Origin \"{origin?.Host ?? "<null>"}\". Closing connection.");
                ...
                while (!_cts.IsCancellationRequested)
                {
                    ...
                    if (fin)
                    {
                        ...
                        else if (opcode == 0x1) // Text
                        {
                            ...
                            if (msglen == 0)
                            {
                                LogInfo("WebSocketServer: msglen == 0");
                            }
                            else if (!mask)
                            {
                                LogInfo("WebSocketServer: Mask bit not set. Closing connection.");
                                break;
                            }
                            else
                            {
                        ...
                        {
                            LogInfo($"WebSocketServer: Unsupported opcode: {opcode}. Closing connection.");
                            break;
                        }
                    }
                    else
                    {
                        LogInfo($"WebSocketServer: Only fin frames are supported. Closing connection.");
                        break;
                    }
                }
            }
        ...
        {
            var commandArray = JsonNode.Parse(message);
            if (commandArray != null)
            {
                ...
                if (commandName != null)
                {
                    ...
                    if (!handled)
                    {
                        LogInfo("WebSocketServer: Unknown command: " + commandName);
                    }
                }
            }
        }
        ...
        private static void LogOnFailure(Task task)
        {
        ...
        private static void LogError(Exception exception) => Console.Error.WriteLine(exception.ToString());
